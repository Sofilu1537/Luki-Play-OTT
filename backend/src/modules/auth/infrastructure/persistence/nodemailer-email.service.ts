  ...
  async sendPasswordReset(
    ...
    const content = `
      <div style="background-color:#f8fafc;padding:20px;border-left:4px solid #6366f1;margin:25px 0;">
        <p style="margin-top:0;font-size:15px;color:#444;">Recibimos una solicitud para restablecer tu contraseña.</p>
        <div style="text-align:center;margin:25px 0;">
          <a href="${resetLink}" style="display:inline-block;padding:14px 28px;background-color:#ffb700;color:#291754;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px;">Restablecer contraseña</a>
        </div>
        <p style="font-size:13px;color:#666;margin-bottom:0;">Este enlace es válido por 1 hora y solo puede usarse una vez.</p>
        <p style="font-size:13px;color:#666;margin-bottom:0;">Si no solicitaste este cambio, ignora este correo.</p>
      </div>
    `;
    await this.send(
      to,
  ...
  private buildLukiEmail(
    ...
    return `
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#333333;line-height:1.6;border:1px solid #e5e5e5;border-radius:8px;overflow:hidden;background-color:#ffffff;">
        ...
        <div style="background-color:#240046;padding:40px;text-align:center;color:#ffffff;">
  ...
  async sendRegistrationRequest(data: RegistrationRequestData): Promise<void> {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#333;border:1px solid #e5e5e5;border-radius:8px;overflow:hidden;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:linear-gradient(135deg,#4f0fc4 0%,#240046 100%);padding:28px 30px;">
          <tr>
            <td>${this.logoImg(40)}</td>
            <td align="right" valign="middle" style="color:#e0d0ff;font-size:18px;font-weight:600;">Nueva solicitud de alta</td>
          </tr>
        </table>
